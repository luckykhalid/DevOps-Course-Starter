...
Follow these steps once to setup CI/CD pipeline with [`Travis CI`](https://travis-ci.com/):
  ...
  * Encrypt these ENV variables in the travis pipeline file. This is one off setup:
  ...
  ```
  * Encrypt DOCKER_PASS env variable in the travis pipeline file. This is one off setup:
  ```bash
      heroku config:set `cat .env | grep SECRET_KEY` -a khalidashraf-todo-app
      heroku config:set `cat .env | grep TRELLO_KEY` -a khalidashraf-todo-app
      heroku config:set `cat .env | grep TRELLO_TOKEN` -a khalidashraf-todo-app
...
### One Time Heroku (CD) Setup  
  * Upload these ENV variables to Heroku application `khalidashraf-todo-app`. You can change this to your own app name in Heroku.
  ```bash
      ...
      heroku config:set `cat .env | grep BOARD_ID` -a khalidashraf-todo-app   
  ```
  * Encrypt DOCKER_PASS env variable in the travis pipeline yml file. This is one off setup:
   ```bash
    travis encrypt --pro DOCKER_PASS="DOCKER_PASS_VALUE" --add # replace DOCKER_PASS_VALUE with the actual value
   ```
